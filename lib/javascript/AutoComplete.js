...
function autoCompleteAttach(element, url) {
    ...
    // These are here for backwards compatibility to the CSS that we used in 2.1.
    // Remove these when the GalleryTheme API gets to 3.x
    ...
    pos[0] += Number(YAHOO.util.Dom.getStyle(container, "right").replace("px", ""));
    pos[1] += Number(YAHOO.util.Dom.getStyle(container, "bottom").replace("px", ""));
