...
function autoCompleteTrigger(id) {
    ...
    if (value == '') return autoCompleteRender(id);
...
function autoCompleteRender(id) {
    ...
    escapedValue = source.value.replace(/([\\\[\]\{\}()^$.*+?|])/g, '\\$1');
    var regexp = new RegExp(escapedValue, "i");
    if (matches.length > 0) {
	for (i = 0; i < Math.min(matches.length, 10); i++) {
	    var row;
	    var newHTML = matches[i].replace(regexp,
		'<span class="autoCompleteHighlight">' + source.value + '</span>');
