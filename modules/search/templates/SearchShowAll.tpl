...
<div id="gsContents">
  ...
  <div id="gsAlbumContents">
	...
	value="{$SearchShowAll.controller}"/>
	...
	<div class="gbAdmin">
	  ...
	  <div class="gbDataEntry">
	    ...
	    <input type="hidden" name="{g->formVar var="form[lastSearchCriteria]"}"
	...
	<div class="giTitle">
	  <p>
	  <input type="submit" name="{g->formVar var="form[action][scan]"}"
	  ...
	  {$SearchShowAll.moduleInfo.name}
	    ...
	    {/if}
	    {if ($form.page > 1)}
	    <input type="submit" name="{g->formVar var="form[action][previousPage]"}"
	     value="{g->text text="&laquo; Back"}"/>
	    {/if}
	    {if ($form.page < $SearchShowAll.maxPages)}
	      <input type="submit" name="{g->formVar var="form[action][nextPage]"}"
