...
<div class="gbBlock">
  ...
  {if isset($form.error.cantWrite.gallery)}
    {g->text text="Cant write to the Gallery .htaccess file"}
  {/if}
  
  {if isset($form.error.cantWrite.embedded)}
    {g->text text="Cant write to the embedded .htaccess file"}
  {/if}
...
<div class="gbBlock">
  <p class="giDescription">
      {g->text text="Short URLs are compiled out of predefined keywords. Modules may provide additional keywords. Keywords are escaped with % (eg: %itemId%). Red keywords have help text available on mouse-over."}
  </p>
  ...
  <tr><th colspan="4"><h2>{$module.name}</h2></th></tr>
  <tr>
    <th> &nbsp; </th>
  ...
  <tr class="{cycle values="gbEven,gbOdd"}">
    ...
    <td>
      {foreach from=$AdminRewrite.shortUrls.$moduleId.rules.$index.keywords key=keyword item=prefs}
        <span title="{$prefs.help|default:""}"{if isset($prefs.help)}style="cursor: help; color: red;"{/if}>
          {$keyword}
        </span>
      {/foreach}
    </td>
