...
<div class="gsContents">
  <div class="gbAdmin">
	...
	{g->text text="URL"}
      ...
      <input type="text" size="60" name="{g->formVar var="form[imageUrl]"}" value="{$form.imageUrl}"
       onfocus="this.style.background='#fff';this.style.color='#000';"
       onblur="this.style.background='#eee';this.style.color='#333';"/>
    ...
    <input type="submit" name="{g->formVar var="form[action][checkUrl]"}"
     value="{g->text text="Check URL"}" class="button"/>
    ...
    arg3="itemId=`$ItemAdmin.item.id`" arg4="addPlugin=ItemAddWebCam"}">{g->text text="change"}</a>
    </strong><br/>
    ...
    <div class="gbBottomFlag">
      <div class="gbButtons">
        <input type="submit" name="{g->formVar var="form[action][addWebCam]"}"
	 value="{g->text text="Add Image"}"/>
      </div>
    </div>
    {/if}
  </div>
