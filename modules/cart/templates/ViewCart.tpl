...
<div id="gsContents">
  ...
  <div id="gsOtherContents">
    ...
    <form action="{g->url}" method="post">
	  ...
	  <input type="submit" name="{g->formVar var="form[action][modify]"}"
	   value="{g->text text="Go"}"/>
        </div>
      </div>
      {/if}
	...
	<table class="gbDataTable">
	  ...
	  <tr class="{cycle values="gbEven,gbOdd"}">
	    <td>
	      <input type="text" size="3" name="{g->formVar var="form[counts][`$item.id`]"}"
	       value="{$form.counts[$item.id]}"/>
	    </td>
