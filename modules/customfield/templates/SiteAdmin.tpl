...
{/literal}
 if (i=='remove') return confirm('{g->text
	 text="WARNING: ALL existing data for this custom field will be deleted!"}');
 else if (i=='album') return confirm('{g->text
	 text="WARNING: Existing data for this custom field on non-album items will be deleted!"}');
 else if (i=='photo') return confirm('{g->text
	 text="WARNING: Existing data for this custom field on non-photo items will be deleted!"}');
{literal}
}
...
<div id="gsAdminContents">
  ...
  <div class="gbAdmin">
    ...
    <p class="giDescription">
      {g->text text="Common fields are available on all Gallery items; Album and Photo fields can be assigned only to items of the appropriate type. Set the display order and visibility of the fields below. Summary fields are typically displayed with thumbnails in an album view. Detail fields are usually shown when viewing the specific item."}
    </p>
...
{*g->url arg1="controller=customfield:CustomFieldSiteAdmin"
         arg2="form[action][down][$set.key]=1" arg3="form[field]=$field"*}
