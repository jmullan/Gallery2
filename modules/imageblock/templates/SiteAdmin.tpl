...
<div id="gsAdminContents">
  ...
  <div class="gbAdmin">
    ...
    <p class="giDescription">
      {g->text text="g2_blocks is a pipe(|) separate list chosen from: randomImage, recentImage, viewedImage, randomAlbum, recentAlbum, viewedAlbum.  g2_show is a pipe(|) separated list chosen from: title, date, views, owner; the value can also be: none.  If either parameter is omitted the site default defined above is used.  Add the parameter g2_heading=on to show the image block heading.  Add the parameter g2_itemId=[id] to limit the item selection to the subtree of the gallery under the album with the given [id]."}
    </p>
  </div>
</div>
